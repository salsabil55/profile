$(document).ready(function () {
  ...
  new WOW().init();
  $(".home-text h1 a").addClass("rubber-animation");
  $(".job-title").addClass("fadein-animation");
...
(function () {
    ...
    for (var i = 0; i < particles.length; i += 1) {
      ...
      for (var j = 0; j < particles.length; j += 1) {
        ...
        if (d < settings.DISTANCE_T) {
          ctx.beginPath();
          ctx.globalAlpha =
            (settings.DISTANCE_T - d) / (settings.DISTANCE_T - 0);
          ctx.lineWidth = 1;
          ctx.moveTo(p.x, p.y);
  ...
  GUI.add(settings, "DISTANCE_T").min(0).max(200).step(10).name("Distance");
  ...
  GUI.add(settings, "AMPLITUDE").min(0).max(20).step(1).name("Amplitude");
